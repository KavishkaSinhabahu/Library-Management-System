...
public class ViewIssuedBooks extends javax.swing.JFrame {
    ...
    public void loadtable() {
        try {
            ResultSet rs = MySQL.search("SELECT * FROM `issue_book` INNER JOIN `books` ON `issue_book`.`books_b_id`=`books`.`b_id` \n"
                    + "INNER JOIN `student` ON `student`.`ac_id` = `issue_book`.`student_ac_id` \n"
                    + "WHERE `issue_book`.`status`='1' AND  `issue_book`.`due_date` >= CURRENT_TIMESTAMP");
            ...
            while (rs.next()) {
                ...
                v.add(name);
                v.add(rs.getString("issue_book.issue_date"));
    ...
    private void initComponents() {
        ...
        jButton2.setText("Print Issued Book List");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try {
            InputStream filepath = getClass().getResourceAsStream("/reports/issueBook.jrxml");
